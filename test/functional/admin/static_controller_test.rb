...
class Admin::StaticControllerTest < ActionController::TestCase
  ...
  test "the page is gated by the grid permission, not the role name" do
    ...
    user = users(:member)
    Group.create!(name: "Committee Aide").tap do |group|
      group.permissions << admin_permissions(:access_committee)
  ...
  test "the Committee role without the permission is refused" do
    # ...and the group name alone grants nothing once the permission is unticked.
    groups(:committee).permissions.delete(admin_permissions(:access_committee))
    sign_in users(:committee)
