...
class LinkHelperTest < ActionView::TestCase
  ...
  test "get_link with index" do
    expected_link = "<a class=\"#{btn_classes(:secondary)}\" title=\"Show All Fault Reports\" href=\"/admin/fault_reports\"><span class=\"no-wrap\"><i class=\"fa-solid fa-th-list\" aria-hidden=\"true\"></i> Show All</span> Fault Reports</a>"
  ...
  test "get_link with new" do
    expected_link = "<a class=\"#{btn_classes(:primary)}\" title=\"New Maintenance Debt\" href=\"/admin/maintenance_debts/new\"><span class=\"no-wrap\"><i class=\"fa-solid fa-plus\" aria-hidden=\"true\"></i> New</span> Maintenance Debt</a>"
  ...
  test "get_link with edit with no_wrap" do
    ...
    expected_link = "<a class=\"#{btn_classes(:secondary)} no-wrap\" title=\"Edit\" href=\"/admin/staffings/1/edit\"><span class=\"no-wrap\"><i class=\"fa-solid fa-pencil-alt\" aria-hidden=\"true\"></i> Edit</span></a>"
  ...
  test "get_link with approve" do
    ...
    expected_link = "<form style=\"display:contents\" class=\"button_to\" method=\"post\" action=\"/admin/opportunities/1/approve\"><input type=\"hidden\" name=\"_method\" value=\"put\" /><button class=\"#{btn_classes(:success)}\" title=\"Approve\" type=\"submit\">Approve</button></form>"
  ...
  test "get_link with reject" do
    news = FactoryBot.create(:opportunity, id: 1)
    expected_link = "<form style=\"display:contents\" class=\"button_to\" method=\"post\" action=\"/admin/opportunities/1/reject\"><input type=\"hidden\" name=\"_method\" value=\"put\" /><button class=\"#{btn_classes(:danger)}\" title=\"Reject\" type=\"submit\">Reject</button></form>"
  ...
  test "get_link with custom action" do
    ...
    expected_link = "<form style=\"display:contents\" class=\"button_to\" method=\"post\" action=\"/admin/maintenance_debts/1/convert_to_staffing_debt\"><input type=\"hidden\" name=\"_method\" value=\"put\" /><button class=\"#{btn_classes(:secondary)}\" title=\"Convert To Staffing Debt\" type=\"submit\">Convert To Staffing Debt</button></form>"
  ...
  test "return link text when condition fails for other action" do
    ...
    assert_equal '<span class="no-wrap"><i class="fa-solid fa-pencil-alt" aria-hidden="true"></i> Edit</span>', get_link(staffing, :edit, condition: false, return_link_text_if_no_permission: true)
  end
  ...
  test "default generate_link_text for instances with name propery" do
    ...
    instance_hash = {
      ...
      edit: '<span class="no-wrap"><i class="fa-solid fa-pencil-alt" aria-hidden="true"></i> Edit</span>',
      destroy: '<span class="no-wrap"><i class="fa-solid fa-trash" aria-hidden="true"></i> Destroy</span>',
      answer: "Answer"
    }
  ...
  test "default generate_link_text for classes" do
    ...
    class_hash = {
      index: '<span class="no-wrap"><i class="fa-solid fa-th-list" aria-hidden="true"></i> Show All</span> Maintenance Debts',
      new: '<span class="no-wrap"><i class="fa-solid fa-plus" aria-hidden="true"></i> New</span> Maintenance Debt'
    }
  ...
  test "get_default_variant" do
    assert_nil get_default_variant(:show)
    assert_equal :primary, get_default_variant(:new)
    assert_equal :danger, get_default_variant(:destroy)
    assert_equal :secondary, get_default_variant(:edit)
    assert_equal :secondary, get_default_variant(:index)
    assert_equal :success, get_default_variant(:approve)
    assert_equal :danger, get_default_variant(:reject)
    assert_equal :secondary, get_default_variant(:answer)
    assert_equal :secondary, get_default_variant(:hexagon)
  end
