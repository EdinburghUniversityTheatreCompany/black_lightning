...
class User < ApplicationRecord
  ...
  # Batch version of reallocate_maintenance_debts — handles N users in 2 queries
  def self.reallocate_maintenance_debts_for_users(users)
    return if users.empty?
    user_ids = users.map(&:id)

    all_debts = Admin::MaintenanceDebt
      .includes(:maintenance_attendance)
      .where(user_id: user_ids, state: :normal)
      .where("due_by >= ? OR maintenance_attendance_id IS NULL", Date.current)
      .order(:user_id, :due_by)
      .to_a
      .group_by(&:user_id)

    all_debt_ids = all_debts.values.flatten.map(&:id)
    all_attendances = MaintenanceAttendance
      .includes(:maintenance_debt)
      .where(user_id: user_ids)
      .where(admin_maintenance_debts: { id: [ nil ] + all_debt_ids })
      .to_a
      .group_by(&:user_id)
    ...
    user_ids.each do |uid|
      debts = all_debts[uid] || []
      attendances = all_attendances[uid] || []
      pairs = [ debts.size, attendances.size ].min
      ...
      (pairs...debts.size).each do |i|
        updates_to_unlink << debts[i].id if debts[i].maintenance_attendance.present?
      end
    end

    Admin::MaintenanceDebt.transaction do
      updates_to_link.each do |u|
        Admin::MaintenanceDebt.where(id: u[:id]).update_all(maintenance_attendance_id: u[:maintenance_attendance_id])
      end
      Admin::MaintenanceDebt.where(id: updates_to_unlink).update_all(maintenance_attendance_id: nil) if updates_to_unlink.any?
    end
  end

  # Batch version of reallocate_staffing_debts — handles N users in 2 queries
  def self.reallocate_staffing_debts_for_users(users)
    return if users.empty?
    user_ids = users.map(&:id)

    all_debts = Admin::StaffingDebt
      .includes(:admin_staffing_job)
      .where(user_id: user_ids, state: :normal)
      .where("due_by >= ? OR admin_staffing_job_id IS NULL", Date.current)
      .order(:user_id, :due_by)
      .to_a
      .group_by(&:user_id)

    all_debt_ids = all_debts.values.flatten.map(&:id)
    all_jobs = Admin::StaffingJob
      .left_joins(:staffing_debt)
      .preload(:staffable)
      .where(user_id: user_ids)
      .where(admin_staffing_debts: { id: [ nil ] + all_debt_ids })
      .to_a
      .group_by(&:user_id)
    ...
    user_ids.each do |uid|
      debts = all_debts[uid] || []
      valid_jobs = (all_jobs[uid] || []).select(&:counts_towards_debt?)
      pairs = [ debts.size, valid_jobs.size ].min
      ...
      (pairs...debts.size).each do |i|
        updates_to_unlink << debts[i].id if debts[i].admin_staffing_job.present?
      end
    end

    Admin::StaffingDebt.transaction do
      updates_to_link.each do |u|
        Admin::StaffingDebt.where(id: u[:id]).update_all(admin_staffing_job_id: u[:admin_staffing_job_id])
      end
      Admin::StaffingDebt.where(id: updates_to_unlink).update_all(admin_staffing_job_id: nil) if updates_to_unlink.any?
    end
  end
