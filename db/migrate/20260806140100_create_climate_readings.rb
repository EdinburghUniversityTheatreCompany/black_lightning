class CreateClimateReadings < ActiveRecord::Migration[8.1]
  ...
  #
  # The Govee API has no history endpoint. It serves "now" and nothing else, so
  # this table IS the history, accumulated by the poll job.
  def change
    create_table :climate_readings do |t|
      ...
      # on_delete: :cascade matches the model's dependent: :delete_all. An orphaned
      # reading is a temperature from nowhere.
      ...
      # What the API returned, plus the unit we BELIEVED at write time, which is
      # what makes a wrong temperature_unit a backfill rather than a permanent
      # hole.
      ...
      # migration is irreversible. A separate add_index reverses as a remove_index
      # BEFORE the drop_table, and MySQL refuses to drop an index a foreign key
      # still needs.
