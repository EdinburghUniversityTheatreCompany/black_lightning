...
class Admin::Proposals::ProposalsControllerTest < ActionController::TestCase
  ...
  test 'someone on the proposal can see debt status' do
    ...
    assert_match '<a class="badge badge-important"', response.body
    assert_match 'In maintenance Debt', response.body

    assert_match 'class="badge badge-important">Has Debtors</span>', response.body, 'The Has Debtors label is absent. Are you sure the label generation did not change? Are you sure one of the users is actually in debt (most likely because there is a maintenance debt label)?'
  end
