...
module Admin
  module Reimbursements
    ...
    class FinanceJsTest < ApplicationSystemTestCase
      ...
      # (f) Editing a Review card then hitting Approve must not silently drop the
      # edit: a dirty Save form pops the three-option confirmation dialog, and
      # Cancel leaves the page and the edit intact (Track J).
      test "a dirty review card intercepts Approve with the unsaved-edits dialog" do
        rebuild_store(expenses: [ airtable_expense_record(id: "recExp1", status: "Pending") ])
      ...
      # (f2) A pristine card never shows the unsaved-edits dialog — the decision's
