module UserHelper
    ...
    def team_member_labels_for(team_member, deadline)
        ...
        output_labels + user_labels_for(team_member.user, deadline)
    end

    # Generate the labels to be shown in the team member list & user profile.
    # Deadline is the proposal deadline, if applicable.
    def user_labels_for(user, deadline)
        output_labels = []
        ...
        if deadline.present? && user.in_debt(deadline)
            on_deadline_debt_message = user.debt_message_suffix(deadline).upcase_first
            ...
            if on_deadline_debt_message != debt_message
                ...
                output_labels << { label_class: "bg-danger", text: link_to(on_deadline_debt_message, admin_debt_path(user)) }
            end
        end
