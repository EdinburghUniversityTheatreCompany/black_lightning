...
class Admin::UsersControllerTest < ActionController::TestCase
  ...
  test "group checkboxes should not be visible for non-admin users" do
    ...
    assert_no_match 'name="user[group_ids][]"', response.body
  end

  test "group checkboxes should be visible for admin users" do
    ...
    assert_match 'name="user[group_ids][]"', response.body
  end

  test "should update user" do
    group = Group.all.first
    attributes = FactoryBot.attributes_for(:user, group_ids: [ group.id ])

    # Explicitly test groups as they are only added to permitted_parms for admins.
    assert_not @user.in_group?(group), "User should not have the group yet before the test."

    put :update, params: { id: @user, user: attributes }

    assert_redirected_to admin_user_path(@user)
    assert @user.in_group?(group), "Group was not assigned to the test"
  end

  # Only admins should be able to update groups, so make sure that committee (who do have edit permission) cannot.
  test "committee cannot update groups on user" do
    sign_out users(:admin)
    ...
    assert_not @user.in_group?(group), "Group was assigned by a committee member when they should not be able to do that."
    assert_redirected_to admin_user_path(@user)
  end
