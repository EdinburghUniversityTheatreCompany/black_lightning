  ...
  module Reimbursements
    class BudgetsControllerTest < ActionController::TestCase
      ...
      test "overview shows a friendly note when there is no unbudgeted spend" do
        ...
        assert_response :success
        # "Unbudgeted spend" is the card title, rendered either way — assert the
        # empty-state SENTENCE, which only appears when the list really is empty.
        assert_includes response.body, "every actual matches a budget"
        assert_empty assigns(:unbudgeted_actuals)
      end

      test "overview does not show the empty-state note when there IS unbudgeted spend" do
        sign_in @user
        ::Reimbursements::EusaActual.create!(nominal_code: "9999", narrative: "Mystery charge",
                                             debit: BigDecimal("42.00"))
        ...
        assert_response :success
        assert_not_includes response.body, "every actual matches a budget"
      end
