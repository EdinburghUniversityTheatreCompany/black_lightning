...
`).hide();
  ...
  const base_url = "/admin/users/autocomplete_list.json";

  const all_users_meta = $('meta[name="all-users"]');

  if (all_users_meta.length) {
    const all_users = all_users_meta.attr("content");
...
var updateUserNames = function() {
  $.each(users, function(i, user) {
    ...
    return user_ids[user_name] = user.id;
  });

};

const userUpdater = function(item) {
  // Update user id
...
const addUserAutocomplete = function() {
  $('.user_name').typeahead({
    source:  user_names,
    updater: userUpdater}).change(function() {
...
jQuery(function() {
  if ($('[name="autocomplete_users_list"]').length === 0) { return; }
