module Reimbursements
  ...
  # store as it arrived.
  #
  # Today that means HEIC/HEIF. iOS photographs default to HEIC, so a producer
  # photographing a receipt on an iPhone used to be rejected outright. We
  # convert to JPEG HERE, before anything is attached, so every downstream
  # consumer sees an ordinary JPEG with no special-casing: the in-page viewer
  # and thumbnail strip, the Gemini extractor and the finance AI check, the
  # SharePoint receipt offload, and the receipts attached to the EUSA BACS
  # email. Storing the HEIC and converting on read would need the same fix in
  # each of those places, and would still hand HEIC to Gemini and to EUSA.
  #
