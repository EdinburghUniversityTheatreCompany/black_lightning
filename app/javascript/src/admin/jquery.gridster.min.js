...
function (a, b, c, d) {
    ...
    g.init = function () {
        ...
        var c = this,
            d = [],
            e = [],
            f = this.colliders || this.$colliders,
            g = f.length,
            h = c.$element.coords().update(b || !1).get();
        while (g--) {
            var i = c.$colliders ? a(f[g]) : f[g],
                j = i.isCoords ? i : i.coords(),
                k = j.get(),
                l = c.overlaps(h, k);
            if (!l) continue;
            var m = c.detect_overlapping_region(h, k);
            if (m === "C") {
                var n = c.calculate_overlapped_area_coords(h, k),
        ...
        return b.sort(function (a, b) {
            return a.region === "C" && b.region === "C" ? a.coords.y1 < b.coords.y1 || a.coords.x1 < b.coords.x1 ? -1 : 1 : a.area < b.area ? 1 : 1
        }), b
...
function (a, b, c, d) {
    ...
    var e = {
        ...
        j = i.prototype;
    j.init = function () {
        this.calculate_positions(), this.$container.css("position", "relative"), this.disabled = !1, this.events(), a(b).bind("resize", throttle(a.proxy(this.calculate_positions, this), 200))
    }, j.events = function () {
        this.$container.on("selectstart", a.proxy(this.on_select_start, this)), this.$container.on(h.start, this.options.items, a.proxy(this.drag_handler, this)), this.$body.on(h.end, a.proxy(function (a) {
            this.is_dragging = !1;
            if (this.disabled) return;
            this.$body.off(h.move), this.drag_start && this.on_dragstop(a)
        }, this))
        ...
        var b = this.get_mouse_pos(a),
            ...
            e = Math.round(this.el_init_offset.left + c - this.baseX),
        ...
        var b, c = f.scrollTop(),
            d = c,
            e = d + this.window_height,
            g = e - 50,
            h = d + 50,
            i = a.mouse_left,
            j = d + a.mouse_top,
            k = this.doc_height - this.window_height + this.player_height;
        ...
        if (this.disabled || b.which !== 1 && !g) return;
        if (this.ignore_drag(b)) return;
        var d = this,
            e = !0;
        ...
        return this.baseX = Math.round(d.left), this.baseY = Math.round(d.top), this.doc_height = a(c).height(), this.options.helper === "clone" ? (this.$helper = this.$player.clone().appendTo(this.$container).addClass("helper"), this.helper = !0) : this.helper = !1, this.scrollOffset = 0, this.el_init_offset = this.$player.offset(), this.player_width = this.$player.width(), this.player_height = this.$player.height(), this.player_max_left = this.$container.width() - this.player_width + this.options.offset_left, this.options.start && this.options.start.call(this.$player, b, {
        ...
        this.disable(), a.removeData(this.$container, "drag")
    }, j.ignore_drag = function (b) {
        return this.options.handle ? !a(b.target).is(this.options.handle) : a.inArray(b.target.nodeName, this.options.ignore_dragging) >= 0
    }, a.fn.drag = function (b) {
        return this.each(function () {
            a.data(this, "drag") || a.data(this, "drag", new i(this, b))
        })
    }
}(jQuery, window, document),
function (a, b, c, d) {
    ...
    g.init = function () {
    ...
    }, g.add_widget = function (b, c, d, e, f) {
        var g;
        c || (c = 1), d || (d = 1), !e & !f ? g = this.next_position(c, d) : (g = {
            col: e,
            row: f
        ...
        return this.$widgets = this.$widgets.add(h), this.register_widget(h), this.add_faux_rows(g.size_y), this.set_dom_grid_height(), h.fadeIn()
    }, g.resize_widget = function (b, c, d) {
        var e = b.coords().grid;
        c || (c = e.size_x), d || (d = e.size_y), c > this.cols && (c = this.cols);
        var f = this.get_cells_occupied(e),
            g = e.size_x,
            h = e.size_y,
            i = e.col,
            j = i,
        ...
        var o = {
            ...
            p = this.get_cells_occupied(o),
            q = [];
        a.each(f.cols, function (b, c) {
            a.inArray(c, p.cols) === -1 && q.push(c)
        });
        var r = [];
        a.each(p.cols, function (b, c) {
            a.inArray(c, f.cols) === -1 && r.push(c)
        });
        var s = [];
        a.each(f.rows, function (b, c) {
            a.inArray(c, p.rows) === -1 && s.push(c)
        });
        var t = [];
        a.each(p.rows, function (b, c) {
            a.inArray(c, f.rows) === -1 && t.push(c)
        }), this.remove_from_gridmap(e);
        if (r.length) {
            var u = [j, e.row, c, Math.min(h, d), b];
            this.empty_cells.apply(this, u)
        }
        if (t.length) {
            var v = [j, e.row, c, d, b];
            this.empty_cells.apply(this, v)
        }
        e.col = j, e.size_x = c, e.size_y = d, this.add_to_gridmap(o, b), b.data("coords").update({
            width: c * this.options.widget_base_dimensions[0] + (c - 1) * this.options.widget_margins[0] * 2,
            height: d * this.options.widget_base_dimensions[1] + (d - 1) * this.options.widget_margins[1] * 2
        }), d > h && this.add_faux_rows(d - h), c > g && this.add_faux_cols(c - g), b.attr({
            "data-col": j,
            "data-sizex": c,
            "data-sizey": d
        });
        if (q.length) {
            var w = [q[0], e.row, q.length, Math.min(h, d), b];
            this.remove_empty_cells.apply(this, w)
        }
        if (s.length) {
            var x = [j, e.row, c, d, b];
        ...
        return this.options.avoid_overlapped_widgets && !this.can_move_to({
            ...
            "data-sizex": b.size_x,
            "data-sizey": b.size_y
        })), a.data("coords", a.coords()), a.data("coords").grid = b, this.add_to_gridmap(b, a), this
        ...
        return this.drag_api = this.$el.drag(c).data("drag"), this
        ...
        var d = this.faux_grid,
            e = this.$player.data("coords").coords;
        ...
        var d = this.get_targeted_columns(this.colliders_data[0].el.data.col),
            e = this.last_cols.length,
            f = d.length,
            g;
        ...
        if (!this.colliders_data.length) return;
        var d = this.get_targeted_rows(this.colliders_data[0].el.data.row),
            e = this.last_rows.length,
            f = d.length,
            g;
        ...
        var h = this.get_widgets_overlapped(this.player_grid_data),
            i = this.widgets_constraints(h);
        this.manage_movements(i.can_go_up, f, g), this.manage_movements(i.can_not_go_up, f, g);
        if (!h.length) {
        ...
        var d = a.extend({}, this.placeholder_grid_data),
            ...
            f = b + d.size_x - 1;
        f > this.cols && (b = b - (f - b));
        var g = this.placeholder_grid_data.row < c,
            h = this.placeholder_grid_data.col !== b;
        ...
        return this.for_each_column_occupied(a, function (a) {
            ...
            d[a] = [];
            while (--h > 0)
                if (this.is_empty(a, h) || this.is_player(a, h) || this.is_widget(a, h) && g[h].is(f)) d[a].push(h), e = h < e ? h : e;
                else break;
        ...
        return this.for_each_column_occupied(a, function (f) {
            ...
            var h = b + 1;
            while (--h > 0) {
                if (this.is_widget(f, h) && !this.is_player_in(f, h) && !g[h].is(a.el)) break;
                !this.is_player(f, h) && !this.is_placeholder_in(f, h) && !this.is_player_in(f, h) && d[f].push(h), h < e && (e = h)
            }
            if (d[f].length === 0) return c = !1, !0;
            d[f].sort()
        }), c ? this.get_valid_rows(a, d, e) : !1
        ...
        while (++h <= f) {
            ...
            a.each(c, function (b, c) {
                a.isArray(c) && a.inArray(h, c) === -1 && (j = !1)
            });
            if (j === !0) {
                i.push(h);
                if (i.length === g) break
        ...
        this.for_each_column_occupied(b, function (a) {
            ...
            g[a] = [], i = h;
            while (i--)
                if (this.is_empty(a, i) && !this.is_placeholder_in(a, i)) g[a].push(i);
                else break;
        ...
        var c = a.isPlainObject(b) ? b : b.coords().grid,
            d = this,
            e = this.gridmap,
            f = c.row + c.size_y - 1,
            g = a([]);
        ...
        return c === 1 ? !1 : (this.for_each_column_occupied(b, function (a) {
            ...
            if (this.is_occupied(a, d) || this.is_player(a, d) || this.is_placeholder_in(a, d) || this.is_player_in(a, d)) return g = !1, !0
        }), g)
        ...
        var e = this.gridmap,
            ...
            h = !0,
            i = b + a.size_x - 1;
        return i > this.cols ? !1 : d && d < c + a.size_y - 1 ? !1 : (this.for_each_cell_occupied(g, function (b, c) {
            var d = this.is_widget(b, c);
            d && (!a.el || d.is(f)) && (h = !1)
        }), h)
    }, g.get_targeted_columns = function (a) {
        var b = (a || this.player_grid_data.col) + (this.player_grid_data.size_x - 1),
            c = [];
        for (var d = a; d <= b; d++) c.push(d);
        return c
    }, g.get_targeted_rows = function (a) {
        var b = (a || this.player_grid_data.row) + (this.player_grid_data.size_y - 1),
            c = [];
        for (var d = a; d <= b; d++) c.push(d);
        return c
        ...
        var l = [],
            ...
            n = {
                "for_each/above": function () {
                    while (m--)
                        if (m > 0 && this.is_widget(d, m) && a.inArray(g[d][m], l) === -1) {
                            h = f.call(g[d][m], d, m), l.push(g[d][m]);
                            if (h) break
                        }
                },
                "for_each/below": function () {
                    for (m = e + 1, i = g[d].length; m < i; m++)
                        if (this.is_widget(d, m) && a.inArray(g[d][m], l) === -1) {
                            h = f.call(g[d][m], d, m), l.push(g[d][m]);
                            if (h) break
                        }
                }
        ...
        var a, b = this.gridmap,
            c = [],
            d = [];
        for (var e = b.length - 1; e >= 1; e--)
            for (a = b[e].length - 1; a >= 1; a--)
                if (this.is_widget(e, a)) {
                    c.push(a), d[a] = e;
                    break
        ...
        for (h = b.cols; h >= 0; h--) c += b.namespace + ' [data-col="' + (h + 1) + '"] { left:' + (h * b.widget_base_dimensions[0] + h * b.widget_margins[0] + (h + 1) * b.widget_margins[0]) + "px;} ";
        ...
        for (var k = 1; k <= b.rows; k++) c += b.namespace + ' [data-sizey="' + k + '"] { height:' + (k * b.widget_base_dimensions[1] + (k - 1) * b.widget_margins[1] * 2) + "px;}";
        for (var l = 1; l <= d; l++) c += b.namespace + ' [data-sizex="' + l + '"] { width:' + (l * b.widget_base_dimensions[0] + (l - 1) * b.widget_margins[0] * 2) + "px;}";
        ...
        for (c = b; c > 0; c--) {
            this.gridmap[c] = [];
            for (d = a; d > 0; d--) this.add_faux_cell(d, c)
        }
        return this
        ...
        f = Array.prototype.slice.call(f, 0), f.length || (f = [0]);
        var g = Math.max.apply(Math, f),
            h = this.options.extra_rows;
        return this.$widgets.each(function (b, c) {
            h += +a(c).attr("data-sizey")
        }), this.cols = Math.max(g, e, this.options.min_cols), this.rows = Math.max(h, this.options.min_rows), this.baseX = (a(b).width() - c) / 2, this.baseY = this.$wrapper.offset().top, this.options.autogenerate_stylesheet && this.generate_stylesheet(), this.generate_faux_grid(this.rows, this.cols)
