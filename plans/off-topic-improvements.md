...
### `bin/bundle` is a pre-Bundler-2 binstub, so every `bin/*` binstub that loads it refuses to run

`bin/kamal deploy` stops at once with "Your `bin/bundle` was not generated by Bundler, so this
...
### Switching a claim to the international rail and back leaves its ex-VAT amount equal to the gross

The finance edit form can now change `payment_method` (2026-09-21). `Expense`'s `before_validation`
mirrors `amount_excl_vat` to the gross on the international rail, and switching back to UK BACS
